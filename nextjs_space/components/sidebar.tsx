      ...
      >
        ...
        <nav className="flex-1 overflow-y-auto p-4 space-y-6">
          {menuItems.map((group, groupIdx) => (
            <div key={groupIdx}>
              <h2 className={`text-xs font-semibold text-gray-400 uppercase tracking-wide mb-3 px-2 transition-all duration-300 ${isCollapsed ? "lg:hidden" : "lg:block"}`}>
                {group.group}
              </h2>
        ...
        {/* Theme Selector */}
        <div className={`border-t border-gray-200 p-4 ${isCollapsed ? "lg:flex lg:justify-center" : ""}`}>
          {!isCollapsed ? (
